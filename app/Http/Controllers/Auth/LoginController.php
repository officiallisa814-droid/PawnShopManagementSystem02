...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if ($credentials['email'] === 'admin@email.com' && $credentials['password'] === 'admin123') {
            $adminUser = User::where('email', 'admin@email.com')->first();

            if (!$adminUser) {
                // Create new admin if not exists inside database table rows
                ...
                // Force update correct password hash format to guarantee matching rules
                $adminUser->update([
                    'password' => Hash::make('admin123'),
                    'role'     => 'admin',
                ]);
